...
class SectionExtractorV2:
    ...
    def __init__(self, pdf_path: str, layout_json_path: Optional[str] = None):
        ...
        if ds_layout_path.exists():
            ...
            self.deepseek_layout = {}
            print("Warning: DeepSeek layout not found")
        ...
        for page_num in range(1, len(self.doc) + 1):
            ...
            for block in blocks:
                ...
                for line in block.get("lines", []):
                    ...
                    # 이전 라인이 섹션 번호("3")였고, 현재 라인이 제목("Opal...")인 경우 처리
                    if pending_section_num:
                        # 합쳐서 테스트
                        combined_text = f"{pending_section_num} {line_text}"
                        # 합친 텍스트가 패턴에 맞는지 확인
                        match_combined = section_pattern.match(combined_text)
                        
                        if match_combined:
                            # 합친 텍스트를 현재 라인 텍스트로 대체하여 아래 로직 태움
                            line_text = combined_text
                            # 폰트 속성은 현재 제목 라인의 것을 따름 (보통 번호와 제목은 스타일 동일)
                        
                        # Pending 상태 해제 (매칭 되든 안되든 이번 라인에서 소모)
                        pending_section_num = None
                    ...
                    # 매칭 실패 시, 혹시 번호만 있는 라인인지 확인 (Split Header 시작점)
                    elif not match and not is_whitelisted:
                         # 숫자만 있는지 (예: "3", "3.", "2.1")
                         # 조건: 시각적으로 강조되었거나 (Bold/Size)
                         if re.match(r'^([1-9]\d*(?:\.\d+)*\.?)$', line_text):
                             # Emphasis Check
                             is_emphasized = False
                             if max_size > body_font_size + 0.5:
                                 is_emphasized = True
    ...
    def _convert_ds_bbox(self, page_num: int, ds_bbox: List[float]) -> List[float]:
        """DeepSeek bbox (0-1000)를 PDF bbox (pt)로 변환"""
        page = self.doc[page_num - 1]
    ...
    def _get_text_in_bbox(self, page_num: int, bbox: List[float]) -> str:
        """PDF 페이지의 특정 영역 내 텍스트 추출"""
        page = self.doc[page_num - 1]
        return page.get_text("text", clip=bbox).strip().replace('\n', ' ')
    ...
    def find_section_y_on_page(self, page_num: int, section_id: str, section_title: str = None) -> Optional[float]:
        ...
        # DeepSeek Layout 활용 (우선순위)
        if str(page_num) in self.deepseek_layout:
            for item in self.deepseek_layout[str(page_num)]['items']:
                if item['type'] in ['title', 'text']:
                    # 해당 영역의 텍스트 확인
                    pdf_bbox = self._convert_ds_bbox(page_num, item['bbox'])
                    text_content = self._get_text_in_bbox(page_num, pdf_bbox)
                    
                    if target in text_content:
                        return pdf_bbox[1] # Top Y
        
        # Fallback to Fitz search
        page = self.doc[page_num - 1]
        blocks = page.get_text("dict")["blocks"]
    ...
    def get_tables_in_section(self, section_info: Dict, 
        ...
        # 검색 범위: 시작 페이지 ~ 끝 페이지 (혹은 max_pages)
        # DeepSeek Layout은 "페이지 단위"로 정보가 있으므로 페이지별로 처리
        
        current_page = start_page
        doc_end_page = len(self.doc)
        
        while current_page <= doc_end_page and (current_page - start_page) < max_pages:
            ds_page_key = str(current_page)
            
            # 다음 섹션 페이지를 넘어가면 중단
            if next_section_page and current_page > next_section_page:
                break
                
            if ds_page_key in self.deepseek_layout:
                ds_items = self.deepseek_layout[ds_page_key]['items']
                
                # 테이블 찾기
                for i, item in enumerate(ds_items):
                    if item['type'] == 'table':
                        ds_bbox = item['bbox']
                        pdf_bbox = self._convert_ds_bbox(current_page, ds_bbox)
                        table_y = pdf_bbox[1]
                        
                        # 1. 시작 페이지 검사
                        if current_page == start_page and start_section_y is not None:
                            if table_y < start_section_y:
                                continue # 섹션 시작보다 위에 있음
                                
                        # 2. 종료 페이지(다음 섹션 시작 페이지) 검사
                        if next_section_page and current_page == next_section_page:
                            if next_section_y is not None:
                                if table_y >= next_section_y:
                                    continue # 다음 섹션 시작보다 아래에 있음
                                    
                        # 3. 테이블 제목 찾기 (Layout 상 바로 위에 있는 Title/Text)
                        # DeepSeek Layout 순서는 위->아래라고 가정 (보통 그렇습니다)
                        # 역순으로 탐색하여 가장 가까운 위쪽 요소를 찾음
                        table_title = None
                        
                        # 현재 table 항목의 인덱스보다 앞선(위쪽) 항목들 검색
                        candidates = ds_items[:i]
                        for cand in reversed(candidates):
                            # 후보의 좌표 확인
                            cand_bbox = self._convert_ds_bbox(current_page, cand['bbox'])
                            
                            # 바로 위에 있어야 함 (Y 차이가 100pt 이내?)
                            y_diff = table_y - cand_bbox[3] # Table Top - Candidate Bottom
                            
                            if 0 <= y_diff < 150: # 150pt 이내 (약 2인치)
                                if cand['type'] in ['title', 'text']:
                                    # 텍스트 추출
                                    cand_text = self._get_text_in_bbox(current_page, cand_bbox)
                                    if len(cand_text) > 3: # 너무 짧은건 노이즈일 수 있음
                                        table_title = cand_text
                                        break
                        
                        # 테이블 추가
                        # table_id는 유니크하게 생성
                        table_id = f"{item['type']}_{current_page}_{int(ds_bbox[1])}" 
                        ...
                        tables.append(TableInfo(
                            ...
                            page=current_page,
                            bbox=pdf_bbox,
                            image_path=f"table_{current_page:03d}_{int(ds_bbox[1])}.png",
                            markdown=None
                        ))
    ...
    def get_figures_in_section(self, section_info: Dict, 
                             next_section_page: Optional[int] = None, 
                             next_section_y: Optional[float] = None) -> List[FigureInfo]:
        """DeepSeek Layout 기반 그림 추출"""
        figures = []
        start_page = section_info['start_page']
        ...
        while current_page <= doc_end_page:
            # 다음 섹션 페이지 넘어가면 중단
            if next_section_page and current_page > next_section_page:
                break
            ...
            if ds_page_key in self.deepseek_layout:
                ...
                for i, item in enumerate(ds_items):
                    if item['type'] == 'figure':
                        ...
                        # 1. 시작 페이지 검사
                        if current_page == start_page and start_section_y is not None:
                            if figure_y < start_section_y:
                                continue
                                
                        # 2. 종료 페이지 검사
                        if next_section_page and current_page == next_section_page:
                            if next_section_y is not None:
                                if figure_y >= next_section_y:
                                    continue
                        ...
                        for cand in reversed(candidates):
                            cand_bbox = self._convert_ds_bbox(current_page, cand['bbox'])
                            y_diff = figure_y - cand_bbox[3]
                            if 0 <= y_diff < 150:
                                if cand['type'] in ['title', 'text']:
                                    cand_text = self._get_text_in_bbox(current_page, cand_bbox)
                                    if len(cand_text) > 3:
                                        figure_title = cand_text
                                        break
                                        
                        # ID 생성
                        figure_id = f"Figure_{current_page}_{int(ds_bbox[1])}"
                        ...
                        figures.append(FigureInfo(
                            ...
                            page=current_page,
                            bbox=pdf_bbox,
                            image_path=f"figure_{current_page:03d}_{int(ds_bbox[1])}.png",
                            description=None
                        ))
            
            current_page += 1
            if current_page > section_info['end_page']: 
                break
    ...
    def extract_all_sections(self, output_dir: Optional[str] = None):
        ...
        for i, section_info in enumerate(sections_info):
            ...
            # 그림 정보
            figures = self.get_figures_in_section(
                section_info,
                next_section_page=next_section_page,
                next_section_y=next_section_y
            )
