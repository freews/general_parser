...
![Untitled Table](../section_images/table_445_128.png)

| | |
| :--- | :--- |
| | as a null-terminated UTF-8 string. A Namespace Admin Label is intended to assist a human administrator in identifying a namespace (e.g., based on the contents of the data stored in the namespace (e.g., "Q4 2018 Financial Records") or the application intended to use the namespace (e.g., "CRM data")). |
| 2.23 | Controller Data Queue (Feature Identifier 21h) |
| | ture allows a host to update the status of the head pointer of a Controller Data Queue (CDQ) and the configuration of a Controller Data Queue Tail event. The CDQ is specified by the Controller |
