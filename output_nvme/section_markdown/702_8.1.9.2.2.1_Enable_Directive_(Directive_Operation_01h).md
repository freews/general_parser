...
![Untitled Table](../section_images/table_579_244.png)

| Reserved | Enable Directive (ENDIR): If this bit is set to '1' and the Directive Type is supported, then the Directive is enabled, unless otherwise specified. If this bit is cleared to '0', then the Directive is disabled. If this bit is set to '1' for a Directive that is not supported, then a status code of Invalid Field in Command shall be returned. |
|---|---|
| | **Figure 650: Enable Directive – Command Specific Status Values** |
| Definition | Stream Resource Allocation Failed: The controller failed to enable the Streams Directive for the specified namespace and Host Identifier due to insufficient resources. |
| Streams (Directive Type 01h, Optional) | Defined user data in a User Data Out command (e.g., logical blocks in a write command) are part of a group of associated data. This information may be used by the controller to store related data in related locations or for other performance enhancements. |
| | controller provides information in response to the Return Parameters operation about the configuration controller that indicates Stream Write Size, Stream Granularity Size, and stream resources at the |


#### Table 2: Untitled Table
![Untitled Table](../section_images/table_579_400.png)
