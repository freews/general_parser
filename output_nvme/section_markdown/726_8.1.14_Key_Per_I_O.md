...
![Untitled Table](../section_images/table_604_266.png)

| Key Per I/O Tag (KPIOTAG): This command is using the Key Per I/O capability. | The same Key Tag value on different namespaces may or may not identify the same encryption key. <br> Refer to the Maximum Key Tag field in the I/O Command Set Independent Identify Namespace data structure (refer to Figure 335) for the supported values. |
|---|---|
| Reserved | |
| Vendor Specific | |
