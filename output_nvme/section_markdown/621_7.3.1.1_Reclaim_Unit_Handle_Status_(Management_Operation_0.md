...
![Untitled Table](../section_images/table_529_244.png)

| 15:14 | number of Reclaim Unit Handle Status Descriptors in the Reclaim Unit Handle Status Descriptor list. |
|---|---|
...
| ... | ... |
| (HSD *32)+15: | **Reclaim Unit Handle Status Descriptor NRUHSD**: The last Reclaim Unit Handle Status Descriptor (refer to the applicable NVM Express I/O Command Set specification), if any. |
| (HSD *32)–16 | |
| | Reclaim Unit Handle Status Descriptors are defined in the NVM Express I/O Command Set specifications, if supported. |
| | |
| | **Command Completion** |
| | When the command is completed with success or failure, the controller shall post a completion queue entry associated I/O Completion Queue indicating the status for the command. |
