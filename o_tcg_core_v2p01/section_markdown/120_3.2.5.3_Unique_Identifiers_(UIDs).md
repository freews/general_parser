...
Each table is also represented by a UID.  A table’s UID is derived from the UID of that table in the
...
a. The first four bytes of a table row’s UID SHALL be the “containing table” portion of the UID
...
assigned by the TPer to that object’s UID, or assigned by this specification or an
SSC for pre-defined tables.  For example, The Table table’s UID SHALL be 0x00
0x00 0x00 0x01 0x00 0x00 0x00 0x01
ii. The UID used to reference the actual table (rather than that table’s row in the
...
0x00’s as the last four bytes of a UID that does not have four 0x00’s at the
beginning are references to a table.
iii. All object UIDs SHALL have their high four bytes be the high four bytes of the
containing table’s UID.  So, references to rows in a table are assigned UIDs based
