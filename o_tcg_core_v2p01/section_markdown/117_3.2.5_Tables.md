...
All persistent data for SPs are stored in tables – the only data for an SP that persists past the end of a
...
(except within a Byte table – see 3.2.5.1), but columns are created only when the table is created.
