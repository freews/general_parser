...
> **Section ID**: 3.3.7.1.4 | **Page**: 74-75
...
requirements.
a. HostExchangeAuthority: The authority that references the Host's Exchange Key – used for
...
authentication – used for authenticating the host; and, for challenge/response
authentication, provides session startup method integrity.
c. SPExchangeAuthority: The authority that references the SP's Exchange Key – used for
exchange of session keys, provides implicit authentication
d. SPSigningAuthority: The authority that references the SP's Signing Key  – used for
...
MAY be referenced in the Host authorities’ Authority table rows. The ability to specify authorities in
