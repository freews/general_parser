...
| :--- | :--- |
| DSSDPSS-1 | If the host selects an DSSD Power State via a DSSD Power State (Feature Identifier C7h) Set Feature, the device shall accept the command and run at the highest powered NVMe Power State whose Maximum Power (MP) is less than or equal to the number of the DSSD Power State in watts (see <a href="https://www.snia.org/tech_activities/tpc/tpc_dssd">DCLP-9 (DSSD Power State Descriptors)</a> and <a href="https://www.snia.org/tech_activities/tpc/tpc_dssd">DSSDPD-3 (NVMe Power State)</a>). |
| DSSDPSS-2 | If the host selects a DSSD Power State via DSSD Power State (Feature Identifier C7h) Set Feature and the number of that DSSD Power State is less than the Minimum DSSD Power State (see <a href="https://www.snia.org/tech_activities/tpc/tpc_dssd">DCLP-8 (Minimum Valid DSSD Power State)</a>), the device shall abort the Set Feature command with Invalid Field in Command status. |
