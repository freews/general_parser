...
| :--- | :--- | :--- | :--- |
| 31:12 | RW | 0h | **Controller Base Address (CBA):** This field specifies the 20 least significant bits of the 64-bit base address for the Persistent Memory Region's controller address range. The Persistent Memory Region's controller base address and its size determine its controller address range.<br>The 64-bit base address specified by this field and PMRMSCL.CBA when the CMSE bit is set to '1' shall be valid only under the following conditions:<br>a) no part of the Persistent Memory Region's controller address range is greater than 2⁶⁴ – 1; and<br>b) if the Controller Memory Buffer's controller memory space is enabled, then the Persistent Memory Region's controller address range does not overlap the Controller Memory Buffer's controller address range. |
