...
| 02:00 | Reservation Acquire Action (RACQA): This field specifies the action that is performed by the command.<br>![](https://i.imgur.com/1X0v0Z0.png) |
| Bytes | Description |
| 07:00 | Current Reservation Key (CRKEY): The field specifies the current reservation key associated with the host. |
| 15:08 | Preempt Reservation Key (PRKEY): If the Reservation Acquire Action is set to 001b (i.e., Preempt) or 010b (i.e., Preempt and Abort), then this field specifies the reservation key to be unregistered from the namespace. For all other Reservation Acquire Action values, this field is reserved.<br>![](https://i.imgur.com/1X0v0Z0.png) |
