...
| **Bits** | **Type** | **Reset** | **Description** |
| 15:14 | RW | 00b | **Shutdown Notification (SHN):** This field is used to initiate a controller shutdown when a power down condition is expected. For a normal controller shutdown, it is expected that the controller is given time to process the controller shutdown before power is removed. For an abrupt shutdown, the host may or may not wait for the controller shutdown to complete before power is removed.<br><br>The controller shutdown notification values are defined as:<br><br>**Value** | **Definition** | |
...
| 13:11 | RW | 000b | **Arbitration Mechanism Selected (AMS):** This field selects the arbitration mechanism to be used. This value shall only be changed when CC.EN is cleared to '0'. The host shall only set this field to supported arbitration mechanisms indicated in CAP.AMS. If this field is set to an unsupported value, then controller behavior is undefined.<br><br>For Discovery controllers, this value shall be cleared to 0h.<br><br>**Value** | **Definition** | |
| | | | 000b | Round Robin |
