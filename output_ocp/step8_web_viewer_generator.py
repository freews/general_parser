...
<head>
    ...
    <style>
        ...
        /* Table Styles */
        .markdown-body table { border-collapse: collapse; width: 100%; margin-bottom: 1em; display: block; overflow-x: auto; }
        .markdown-body th, .markdown-body td { border: 1px solid #d1d5db; padding: 8px 12px; }
        .markdown-body th { background-color: #f9fafb; font-weight: 600; text-align: left; }
    ...
    <div id="app" class="flex h-full overflow-hidden">
        ...
        <div class="w-80 bg-white border-r flex flex-col shadow-sm z-10">
            <div class="p-4 border-b bg-gray-50">
                <h1 class="font-bold text-lg text-indigo-700 truncate" :title="docTitle || 'Spec Library'">{{ docTitle || 'Spec Library' }}</h1>
            ...
            <main class="flex-1 overflow-y-auto p-8 bg-white max-w-5xl mx-auto w-full shadow-sm m-4 rounded-lg">
                ...
                <div v-else class="space-y-12">
                    ...
                    <section class="pt-8 border-t">
                        ...
                        <!-- Original Pages -->
                        <div class="space-y-4">
                             <div v-if="currentSection.pages && currentSection.pages.length > 0">
                                <p class="mb-3 text-sm text-gray-600">Referenced Pages: <span class="font-mono bg-gray-100 px-1 rounded">{{ currentSection.pages.join(', ') }}</span></p>
                                <div class="space-y-6">
                                    <div v-for="page in currentSection.pages" :key="page" class="border rounded-lg overflow-hidden shadow-sm bg-gray-50">
                                        <div class="bg-gray-100 px-4 py-2 border-b text-xs font-semibold text-gray-500 flex justify-between">
                                            <span>Page {{ page }}</span>
                                            <a :href="'../page_images/' + page.toString().padStart(4, '0') + '_page.png'" target="_blank" class="text-indigo-600 hover:text-indigo-800">Open Image</a>
                                        </div>
                                        <img :src="'../page_images/' + page.toString().padStart(4, '0') + '_page.png'" loading="lazy" class="w-full h-auto" />
                                    </div>
                                </div>
                             </div>
                             
                             <div v-else class="p-4 border rounded bg-gray-50 text-gray-600">
    ...
    <script>
        ...
        createApp({
            ...
            methods: {
                async loadData() {
                    try {
                        const res = await fetch('data/summary.json?t=' + new Date().getTime());
                        const data = await res.json();
                        
                        if (Array.isArray(data)) {
                             // Legacy format support
                             this.sections = data;
                             this.docTitle = "Document";
                             this.buildTree(data);
                        } else {
                             // New format
                             this.sections = data.sections;
                             this.docTitle = data.title;
                             this.buildTree(data.sections);
                        }
