...
| 10h | **Host-Determined Failure Record**: A failure record (e.g., the reason the host has flagged a failure for an NVMe Storage Device (refer to the NVM Express Management Interface Specification)) FRU may be used for failure analysis) as a UTF-8 string. |
| 11h to 17h | Reserved |
