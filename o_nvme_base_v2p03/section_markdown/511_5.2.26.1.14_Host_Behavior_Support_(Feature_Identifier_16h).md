...
| | |
| Bytes | Description |
| | **Copy Descriptor Formats Enable (CDFE):** The bits in this field that are used (i.e., are neither reserved nor unused) enable the corresponding Copy Descriptor Formats (e.g., the Copy Descriptor Format 2h Enable (CDF2E) bit enables Copy Descriptor Format 2h). Bits in this field that this specification defines as unused (i.e., bits 0:1) correspond to Copy Descriptor Formats (refer to the NVM Express NVM Command Set Specification) that are always enabled if they are supported. |
| 05:04 | | |
| | Bits | Description |
| | 15:5 | Reserved |
