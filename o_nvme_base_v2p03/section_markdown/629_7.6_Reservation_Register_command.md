...
| Bytes | Description |
| 07:00 | **Current Reservation Key (CRKEY):** If the Reservation Register Action field is 001b (i.e., Unregister Reservation Key) or 010b (i.e., Replace Reservation Key), then this field contains the current reservation key associated with the host. For all other Reservation Register Action values, this field is reserved. <br> The controller ignores the value of this field when the Ignore Existing Key (IEKEY) bit is set to '1'. |
| Bytes | Description |
